...
- tipo "compra" — comprar un insumo PARA UN EVENTO PUNTUAL (el usuario menciona a qué evento va, ej "para el evento Cohen", "para el sábado"). Se consume directo en ese evento. Por defecto se descuenta de caja operativa — pero si el usuario dice explícitamente que la pague con el anticipo de ese mismo evento (ej "pagala con la seña del evento"), usá cuenta_origen "anticipos_comprometidos". No hace falta preguntar de qué evento sale ese anticipo: siempre es el mismo evento de la compra:
  evento_id, marca, proveedor, presentacion (ej "750ml"), ml_por_envase (número), cantidad (número), precio_unitario_real (número, precio por unidad), fecha_compra (YYYY-MM-DD), cuenta_origen ("caja_operativa" | "anticipos_comprometidos", opcional, default caja_operativa)

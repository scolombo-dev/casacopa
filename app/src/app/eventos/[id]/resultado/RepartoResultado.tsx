...
import { crearReparto, eliminarReparto } from './actions'

type Reparto = { id: string; destinatario: string; monto: number; fecha: string; notas: string | null }

export default function RepartoResultado({ eventoId, resultadoNeto, repartos }: {
  eventoId: string
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await crearReparto({ evento_id: eventoId, destinatario, monto, fecha, notas })
      if (res.error) { setError(res.error); return }
      setDestinatario(''); setMonto(0); setNotas('')
      setAgregando(false)
      router.refresh()
    })
  }
  ...
  return (
    <div className="mb-6">
            ...
            {repartos.map(r => (
              <div key={r.id} className="flex items-center justify-between px-4 py-2.5 text-sm group">
                <div>
                  ...
                  <span className="text-gray-400 ml-2">· {formatFecha(r.fecha)}</span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => startTransition(async () => { await eliminarReparto(r.id, eventoId); router.refresh() })}
                    disabled={pending}
      ...
      {agregando && (
        <form onSubmit={handleSubmit} className="print:hidden mt-3 bg-gray-50 border rounded-xl p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="button" onClick={() => { setAgregando(false); setError(null) }}
